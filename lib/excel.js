...
function encontrarColumna(filaEncabezados, prefijo) {
  let columna = null;
  filaEncabezados.eachCell((celda, numeroColumna) => {
    const valor = normalizarEncabezado(celda.value);
    if (valor && valor.startsWith(prefijo)) columna = numeroColumna;
  });
  return columna;
}

function encontrarColumnaPrecios(fila) {
  return encontrarColumna(fila, "precio");
}

function encontrarColumnaItemNo(fila) {
  return encontrarColumna(fila, "item");
}
